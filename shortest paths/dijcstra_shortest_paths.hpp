...
auto elogv_dijcstra_shortest_paths(const Graph& graph, const typename Graph::vertex_descriptor& start) {
  ...
  while (!available_vertexes.empty()) {
    typename Graph::vertex_descriptor vertex = available_vertexes.begin()->second;
    ...
    for (auto neighbour = graph.neighbours_begin(vertex); neighbour != graph.neighbours_end(vertex); ++neighbour) {
      if (shortest_paths[*neighbour] > shortest_paths[vertex] + graph.edge_weight(vertex, *neighbour)) {
        available_vertexes.erase({shortest_paths[*neighbour], *neighbour});
        shortest_paths[*neighbour] = shortest_paths[vertex] + graph.edge_weight(vertex, *neighbour);
...
auto v2_dijcstra_shortest_paths(const Graph& graph, const typename Graph::vertex_descriptor& start) {
  ...
  while (!unused_vertexes.empty()) {
    ...
    for (auto vertex = unused_vertexes.begin(); vertex != unused_vertexes.end(); ++vertex) {
      if (shortest_paths[*nearest_vertex] > shortest_paths[*vertex])
        nearest_vertex = vertex;
    }

    for (auto neighbour = graph.neighbours_begin(*nearest_vertex); neighbour != graph.neighbours_end(*nearest_vertex); ++neighbour)
      shortest_paths[*neighbour] = std::min(shortest_paths[*neighbour], shortest_paths[*nearest_vertex] + graph.edge_weight(*nearest_vertex, *neighbour));
