...
/**
 * @brief Находит кратчайший путь между двумя вершинами в графе с использованием
 * двойного алгоритма Дейкстры.
 *
 * Этот метод выполняет два параллельных обхода: один от начальной вершины к
 * конечной, а другой — от конечной к начальной. Это позволяет эффективно
 * находить кратчайший путь в графах, особенно в разреженных графах.
 *
 * @tparam Graph Тип графа, который будет обрабатываться.
 * @param graph Граф, в котором будет найден кратчайший путь.
 * @param start Начальная вершина для поиска кратчайшего пути.
 * @param finish Конечная вершина для поиска кратчайшего пути.
 * @return Кратчайшее расстояние между начальной и конечной вершинами.
 */
template <typename Graph>
auto DoubleDijcstraShortestPath(
  ...
  // Инициализация расстояний и множеств вершин
  std::unordered_map<typename Graph::vertex_descriptor, typename Graph::weight>
      shortest_paths_forward;
  std::unordered_map<typename Graph::vertex_descriptor, typename Graph::weight>
      shortest_paths_reverse;
  ...
  for (;;) {
    ...
    for (auto vertex = unused_vertexes_forward.begin();
         vertex != unused_vertexes_forward.end(); ++vertex) {
      if (shortest_paths_forward[*nearest_vertex_forward] >
          shortest_paths_forward[*vertex])
    ...
    for (auto neighbour = graph.neighbours_begin(*nearest_vertex_forward);
         ...
         ++neighbour) {
      shortest_paths_forward[*neighbour] = std::min<typename Graph::weight>(
          shortest_paths_forward[*neighbour],
          shortest_paths_forward[*nearest_vertex_forward] +
              graph.edge_weight(*nearest_vertex_forward, *neighbour));
    }
    used_vertexes_forward.insert(*nearest_vertex_forward);
    if (used_vertexes_reverse.find(*nearest_vertex_forward) !=
        used_vertexes_reverse.end())
      break;
    ...
    for (auto vertex = unused_vertexes_reverse.begin();
         vertex != unused_vertexes_reverse.end(); ++vertex) {
      if (shortest_paths_reverse[*nearest_vertex_reverse] >
          shortest_paths_reverse[*vertex])
    ...
    for (auto neighbour = graph.vertexes_begin(reverse_filter);
      ...
      shortest_paths_reverse[*neighbour] = std::min<typename Graph::weight>(
          ...
          shortest_paths_reverse[*nearest_vertex_reverse] +
              graph.edge_weight(*neighbour, *nearest_vertex_reverse));
    }

    used_vertexes_reverse.insert(*nearest_vertex_reverse);
    if (used_vertexes_forward.find(*nearest_vertex_reverse) !=
        used_vertexes_forward.end())
      break;
  ...
  for (auto forward_connect : used_vertexes_forward) {
    for (auto reverse_connect : used_vertexes_reverse) {
      if (graph.edge_weight(forward_connect, reverse_connect) < kInf) {
        shortest_path = std::min<typename Graph::weight>(
            shortest_path,
            shortest_paths_forward[forward_connect] +
                graph.edge_weight(forward_connect, reverse_connect) +
                shortest_paths_reverse[reverse_connect]);
      }
    }
  }

  return shortest_path;  // Возвращаем кратчайшее расстояние между начальной и
                         // конечной вершинами
}
